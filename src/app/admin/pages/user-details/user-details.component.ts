...
import { ConfirmDialogComponent, InputComponent, InputSelectComponent } from '@shared/components';
import { BehaviorSubject, combineLatest, debounceTime, map, Observable, shareReplay, startWith } from 'rxjs';
import isEqual from "fast-deep-equal"
import { MatDialog } from '@angular/material/dialog';
import { MatSnackBar, MatSnackBarConfig } from '@angular/material/snack-bar';
@Component({
	selector: 'professional-page-user-details',
	standalone: true,
	imports: [
		MatButton,
		MatIcon,
...
export class UserDetailsComponent implements OnInit {
	public userDetails$ = new BehaviorSubject<IUserDetails | undefined>(undefined);
	...
	public userPlatformInformation = this.formBuilder.group({
		id: this.formBuilder.control(0, { nonNullable: true, validators: [Validators.required] }),
		...
		avatar: this.formBuilder.control("", { nonNullable: false, validators: [] }),
		apiKey: this.formBuilder.control("", { nonNullable: true, validators: [Validators.required] }),
		apiSecret: this.formBuilder.control("", { nonNullable: true, validators: [Validators.required] }),
		statusId: this.formBuilder.control(0, { nonNullable: true, validators: [Validators.required] }),
		channelId: this.formBuilder.control(0, { nonNullable: true, validators: [Validators.required] }),
		createdAt: this.formBuilder.control("", { nonNullable: true, validators: [Validators.required] }),
		updatedAt: this.formBuilder.control("", { nonNullable: true, validators: [Validators.required] }),
		deletedAt: this.formBuilder.control("", { nonNullable: false, validators: [] }),
	})

	public userPersonalInformation = this.formBuilder.group({
		identificationType: this.formBuilder.control("", { nonNullable: true, validators: [Validators.required] }),
		identification: this.formBuilder.control("", { nonNullable: true, validators: [Validators.required] }),
	...
	ngOnInit(): void {
		...
		this.userFacadeService.userFound.subscribe((userDetails: IUserDetails | undefined) => {
			if (userDetails) {
		...
		combineLatest(
			[
				...
				this.userPlatformInformation.valueChanges.pipe(map((values) => ({
					...values,
		...
		).pipe(map(([a, b]) => ({ ...a, ...b })), this.dirtyCheck(this.userDetails)).subscribe(this.isDirty.set)
	}
	...
	public dirtyCheck<IUserDetails>(source: Observable<IUserDetails>) {
		return function <IUserDetails>(valueChanges: Observable<IUserDetails>): Observable<boolean> {
			...
			if (value) {
				this.userFacadeService.delete(userId, true).subscribe({
					next: () => {
						...
						this.handleGoBack()
						this.openSnackBar("Usuario eliminado con exito", 3000, "fill", "success")

					},
					error: (err) => {
						this.openSnackBar(err.message ?? "El usuario no se pudo eliminar", 3000, "fill", "error");
					}
				})
			}
		})
	...
	public handleEdit() {
		...
		this.userFacadeService.update({ ...allFormsData }).subscribe({
			next: () => {
				this.openSnackBar("Los cambios fueron aplicados exitosamente", 3000, "fill", "success");
			},
			error: (err) => {
				this.openSnackBar(err.message ?? "Los cambios no fueron aplicados", 3000, "fill", "error");
			}
		})
	}
