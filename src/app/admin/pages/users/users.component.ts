...
import { ConfirmDialogComponent, DataTableComponent, ButtonComponent, UserFormModalComponent } from '@shared/components';
...
@Component({
	selector: 'professional-page-users',
	standalone: true,
	imports: [
		DataTableComponent,
		AsyncPipe,
...
export class UsersComponent implements OnInit {
	...
	ngOnInit(): void {
		...
		this.userFacadeService.allUsers.subscribe({ next: this.handleGetResponse.bind(this), error: this.handleGetError.bind(this) })
	...
	public handleTableAction({ id, action }: { id: number, action: string }): void {
		if (action === "delete") {
	...
	public handleCreate(): void {
		this.dialog.open(UserFormModalComponent, { width: "100%", maxWidth: this.getModalMaxWidth() })
	}
