...
import { AfterViewInit, Component, EventEmitter, Input, OnChanges, OnInit, Output, SimpleChanges, ViewChild, signal } from '@angular/core';
...
import { DataSourceType, DataTableActionType } from '@core/types/data-table';
import { InputComponent } from '../input/input.component';
@Component({
	selector: 'professional-page-data-table',
	standalone: true,
	imports: [
		MatTableModule,
		MatSort,
...
export class DataTableComponent implements OnInit, OnChanges, AfterViewInit {
	...
	@Output() public _handleAction = new EventEmitter<{ id: number, action: string }>();
	...
	ngOnInit(): void {
		this.dataSource = this.dataSource.map((data: DataSourceType<unknown>) => ({ ...data, acciones: this.actions }));
	}

	ngOnChanges(changes: SimpleChanges): void {
		console.log(changes)
		if (changes['dataSource'] && changes['dataSource'].currentValue) {
			this.dataSource = changes['dataSource'].currentValue.map((data: DataSourceType<unknown>) => ({ ...data, acciones: this.actions }));
	...
	public handleSortData({ active, direction }: { active: string, direction: string }): void {
		this.matData.data = this.dataSource.sort((a: { [key: string]: unknown }, b: { [key: string]: unknown }) => {
			const activePropertyA = a[active] as (string | number);
			const activePropertyB = b[active] as (string | number);

			return this.compare(activePropertyA, activePropertyB, direction === "asc")
		})
	}
